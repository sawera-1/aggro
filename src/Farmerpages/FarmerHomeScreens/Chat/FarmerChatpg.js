  ...
  const unsubscribeUser = firestore()
    ...
    .onSnapshot(
      (doc) => {
        ...
        if (role === "farmer") {
          //  farmers + experts (hide admins)
          query = query.where("role", "in", ["farmer", "expert"]);
        } else if (role === "expert") {
          //  only experts
          query = query.where("role", "==", "expert");
        } else {
          // Admin → empty
          setUsers([]);
          return;
        ...
        const unsubscribeUsers = query.onSnapshot(
          ...
          (error) => {
            ...
            if (error.code === 'firestore/permission-denied') {
              Alert.alert('Permission Error', 'You do not have permission to access user data. Please check your authentication.');
            } else {
      ...
      (error) => {
        ...
        if (error.code === 'firestore/permission-denied') {
          Alert.alert('Permission Error', 'You do not have permission to access your user data. Please check your authentication.');
        } else {
