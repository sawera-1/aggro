...
export default function ChannelMsgScreen({ route, navigation }) {
  ...
  // Fetch followed timestamp on mount
  useEffect(() => {
    const fetchFollowedAt = async () => {
      try {
        ...
        if (doc.exists) {
          setFollowedAt(doc.data().followedAt?.toDate() || new Date());
        } else {
          // User hasn't followed the channel yet
          setFollowedAt(null);
        }
      } catch (err) {
        console.log("Error fetching followed timestamp:", err);
        setFollowedAt(null);
      }
    };
    fetchFollowedAt();
        ...
        const allMessages = snapshot.docs
          .map(doc => {
            ...
            return {
              ...
              readBy: data.readBy || {},
              // Blue tick only if someone OTHER than sender read it
              isRead: !!(data.readBy && Object.keys(data.readBy).some((uid) => uid !== data.senderId)),
              type: data.type || 'text',
          ...
          // Only include messages sent after the user followed the channel
          .filter(msg => msg.createdAt >= followedAt);

        setMessages(allMessages);
      });

    return () => unsubscribe();
  }, [channelId, currentUser, followedAt]);
    ...
    const fetchChannelData = async () => {
      ...
      try {
        ...
        setChannelInfo({
          ...
          createdAt: data.createdAt ? data.createdAt.toDate() : null, // <-- add this
        });
  ...
  const handleShareContact = async () => {
    try {
      ...
      if (perm === 'undefined' || perm === 'denied') perm = await Contacts.requestPermission();
      if (perm !== 'authorized') return Alert.alert('Permission needed', 'Contacts permission is required.');
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={[styles.topBar, { alignItems: 'center' }]}>
        ...
        <TouchableOpacity
          onPress={() =>
            navigation.navigate("ChannelDes", {
              ...
              channelImage: channelInfo.imageUrl,
              channelDescription: channelInfo.description,
              createdBy: channelInfo.createdBy,
              dateCreated: channelInfo.createdAt
                ? channelInfo.createdAt.toLocaleString()
                : "N/A"
            })
        ...
        <View style={{ flex: 1, marginLeft: 5, flexShrink: 1 }}>
          <Text style={styles.channelName} numberOfLines={1}>
            {channelInfo.name}
          </Text>
          <Text style={styles.channelDesc} numberOfLines={1} ellipsizeMode="tail">
